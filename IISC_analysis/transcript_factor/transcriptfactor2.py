...
#violinplot(dftranscriptionrand)

def violinplotcompare(df1,df2,label1="transcription",label2="protien"):
    plt.figure(figsize=(10, 6))
    sns.violinplot(x='Essentiality test', y="DeepLOF_score", data=df1, palette='Set3',label=label1)
    ax = sns.violinplot(x='Essentiality test', y="DeepLOF_score", data=df2, palette='pastel',label = label2)
    ...
    plt.title('Violin Plot of deepLOF Scores by Essentiality Test')
    plt.show()
#violinplotcompare(dftranscriptionrand,dfprotien)
