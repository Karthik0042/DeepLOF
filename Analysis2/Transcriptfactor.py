...
protien_complex_data = merged_data[(merged_data['transcript_factor'] == 1) & (merged_data['protein_complex'] == 0)]
...
for index, essential_gene in essential_genes_df.iterrows():
    ...
    similar_non_essential_genes = non_essential_genes_lof.loc[
        (non_essential_genes_lof["exp_lof"] >= essential_exp_lof-5) &
        (non_essential_genes_lof['exp_lof'] <= essential_lof+5)
...
print(final_df[["obs_lof","exp_lof","Essentiality test","DeepLOF_score","protein_complex","transcript_factor"]])
...
print("accuracy",count*100/len(deeplof_scores_list_essential))
print(len(final_df))


# Create a function to plot essential and non-essential genes along with their DeepLOF scores
